 ...
 * Durable agent runs.
 *
 * A run is a row in `runs`, not a request. The browser stream and the cron
 * worker are two clients of the same engine: whoever holds the lease pushes the
 * conversation forward, saves everything to the database, and releases the
 * lease. Closing the tab therefore never stops a task.
 */

export type Send = (event: StreamEvent) => void;

/** Live aborts for runs executing inside THIS worker instance. */
export const activeRuns = new Map<string, AbortController>();

export type RunOutcome = {
  status: "complete" | "cancelled" | "error" | "handoff" | "waiting";
...
const NETWORK_RULES = `## Network from the sandbox
- The sandbox has unrestricted outbound access; it is opened automatically at boot. A single TLS reset
  or timeout is almost never "the sandbox blocks it".
- On any connection failure run sandbox_network_check with the failing URL. It reports DNS, TCP, TLS and
  HTTP separately, and reopens the egress policy. Read which layer actually failed and react to that layer.
- If the target rejects the sandbox itself (TLS reset at handshake, 403 from an edge/WAF), retry through
  http_fetch — it performs the request from the server, outside the sandbox — before telling the user the
  environment is at fault. Also try: different user-agent/headers, HTTP/1.1 (curl --http1.1), IPv4 only
  (curl -4), and the site's API/JSON endpoint.
- Only claim an environment/network limitation after sandbox_network_check AND http_fetch both failed, and
  quote their real output.`;
...
export type RunSpec = {
  ...
  runId: string;
  send?: Send;
  /** Stop cleanly and hand the run back to the queue after this timestamp. */
  deadlineAt?: number;
  /** Model turns allowed in this slice. */
  maxRounds?: number;
};

/**
 * Execute one slice of a run. Returns "handoff" when work remains — the caller
 * leaves the run row in `running` so the cron worker picks it up again.
 */
export async function executeRun(spec: RunSpec): Promise<RunOutcome> {
  const { chatId, requestId, runId } = spec;
  const send: Send = spec.send ?? (() => {});
  const deadlineAt = spec.deadlineAt ?? Date.now() + 9 * 60_000;
  const maxRounds = spec.maxRounds ?? 48;
  ...
  const { data: chat } = await db
    ...
    .eq("id", chatId)
    .maybeSingle();
  if (!chat) return { status: "error", messageId: null, message: "Chat not found" };
  ...
  const { data: existingMessage } = await db
    .from("messages")
    .select("id, content, planning, thinking, events, status")
    .eq("request_id", requestId)
    .maybeSingle();

  let messageId = (existingMessage as { id: string } | null)?.id ?? null;
  let answer = ((existingMessage as { content?: string } | null)?.content ?? "") as string;
  ...
  if (!messageId) {
    const { data: placeholder } = await db
      ...
      .select("id")
      .single();
    messageId = (placeholder as { id: string } | null)?.id ?? null;
  }
  if (messageId) await db.from("runs").update({ message_id: messageId } as never).eq("id", runId);
  ...
  const { data: newestHistory } = await db
    .from("messages")
    .select(
      "id, role, content, message_attachments(file_name, mime_type, storage_path, extracted_text)",
    )
    .eq("chat_id", chatId)
    .order("seq", { ascending: false })
    .limit(16);
  const history = (newestHistory ?? [])
...
- Your run is durable: it keeps executing on the server even if the user closes the tab, so never rush or
  cut a task short to "finish before the connection drops".
...
- Keep the answer concise unless the user asks for detail.` : THINKING_RULES}
...
## Sandbox
${sandboxKey ? `A real Daytona Linux sandbox is connected at /home/daytona/project. All sandbox tools are active.
...
You may only manage the sandbox of this chat; never touch other sandboxes or projects.` : `No sandbox is configured, so you cannot execute code. Answer with code blocks and tell the user
...
${skillBlock || "(none enabled)"}`;
  ...
  const finishRun = async (status: string, error?: string) => {
    await db
      .from("runs")
      .update({
        status,
        ...
        last_heartbeat: new Date().toISOString(),
        updated_at: new Date().toISOString(),
        ...(error ? { last_error: error.slice(0, 800) } : {}),
      } as never)
      .eq("id", runId);
  };
  ...
  try {
    ...
    const lovableProviderOptions =
      isLovableGateway(provider) && modelRow.model_id === "openai/gpt-5.4"
        ? { lovable: { reasoningEffort: "none", service_tier: "priority" } }
        : undefined;
    ...
    const unfinishedSteps = () => {
      const last = [...events]
        .reverse()
        .find((e): e is Extract<StreamEvent, { type: "progress" }> => e.type === "progress");
      if (!last) return null;
    ...
    const waitingForUser = () =>
      [...events]
        .reverse()
        .some((e, i) => i < 6 && (e.type === "ask-user" || e.type === "secret-request"));

    const continueInstruction = (pending: { label: string }[]) =>
    ...
    if (resuming) {
      ...
      if (waitingForUser()) {
        await queuePersist("streaming");
        await finishRun("waiting_user");
        return { status: "waiting", messageId };
      }
      if (!pending) {
        await queuePersist("complete");
        await finishRun("done");
        return { status: "complete", messageId };
      }
      if (answer.trim()) convo = [...convo, { role: "assistant", content: answer }];
      convo = [...convo, { role: "user", content: continueInstruction(pending) }];
    ...
    for (let round = 0; round <= maxRounds; round++) {
      ...
      // Out of time or rounds for this slice: keep the run alive in the queue.
      if (round === maxRounds || Date.now() > deadlineAt) {
        handoff = true;
        break;
      }
      ...
      convo = [
        ...
        { role: "user", content: continueInstruction(pending) },
      ];
      answer = answer.trimEnd() + (answer.trim() ? "\n\n" : "");
    }
    ...
    planning =
      events
        .filter(
          (e): e is Extract<StreamEvent, { type: "thought-delta" }> => e.type === "thought-delta",
        )
        .map((e) => e.text)
        .join("\n\n")
        .slice(0, 4000) || planning;
    ...
    if (handoff) {
      ...
      await db
        ...
        .update({
          status: "running",
          round: 0,
          // Free the lease right away so the next tick continues the work.
          lease_until: new Date().toISOString(),
          last_heartbeat: new Date().toISOString(),
          updated_at: new Date().toISOString(),
        } as never)
        .eq("id", runId);
      return { status: "handoff", messageId };
    }

    await queuePersist(cancelled ? "cancelled" : "complete");
    await finishRun(cancelled ? "cancelled" : waitingForUser() ? "waiting_user" : "done");

    await db.from("chats").update({ updated_at: new Date().toISOString() }).eq("id", chatId);
    await audit("chat.generate", "chats", chatId, { model: modelRow.model_id, cancelled });

    if (cancelled) send({ type: "cancelled" });
    send({ type: "assistant-finish", messageId: messageId ?? requestId });
    return { status: cancelled ? "cancelled" : "complete", messageId };
  } catch (error) {
    const aborted =
      cancelled || (error instanceof Error && /abort|cancel/i.test(error.name + error.message));
    if (aborted) {
